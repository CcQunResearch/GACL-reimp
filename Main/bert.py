...
def load_bert(bert_path):
    bert_config_file = os.path.join(bert_path, 'config.json')

    bert_tokenizer = bt.from_pretrained(bert_path)
    bert_model = bm.from_pretrained(bert_path)
    bert_config = bc.from_json_file(bert_config_file)
    return bert_tokenizer, bert_model, bert_config
